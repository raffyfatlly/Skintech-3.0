...
const MODEL_IMAGE = 'gemini-2.5-flash-image'; // Nano Banana / Flash Image

// Permissive Safety Settings for Medical/Skin Analysis
...
export const generateRetouchedImage = async (imageBase64: string): Promise<string> => {
    return runWithTimeout<string>(async (ai) => {
        // Strict split at comma to remove data URI header (Identical logic to analyzeFaceSkin)
        const base64Data = imageBase64.includes(',') ? imageBase64.split(',')[1] : imageBase64;

        // Simplify prompt to reduce complexity for the image model
        const prompt = "High quality portrait photo editing. Output the exact same image but with smoother skin texture, reduced acne, and reduced redness. Keep face structure, eyes, and hair 100% identical. Natural finish.";
        ...
        const response = await ai.models.generateContent({
            ...
            // CRITICAL: Disable safety filters that block face generation/editing in production
            config: {
        ...
        // Debugging: If text returned instead of image (refusal)
        const textPart = respParts?.find(p => p.text);
        if (textPart) console.warn("Gemini Refusal:", textPart.text);
